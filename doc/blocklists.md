...
- `hosts` - A blocklist in hosts-file format. If a non-zero IP address is provided for a record, the response is spoofed rather than returning NXDOMAIN. Names pointed at an unspecified address (`0.0.0.0` or `::`) have nothing to spoof and are held the same way the `domain` format holds its rules, so a list written that way costs what it would in that format. A list that sinkholes to `127.0.0.1` instead is spoofing that address, and its names are held individually so that they can be answered with it. PTR queries are answered from the names that carry an address; no reverse lookup is recorded for an unspecified address, since every blocked name in a list would share the one entry.
