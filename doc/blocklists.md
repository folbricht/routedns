...
- `hosts` - A blocklist in hosts-file format. If a non-zero IP address is provided for a record, the response is spoofed rather than returning NXDOMAIN. Names pointed at an unspecified address (`0.0.0.0` or `::`), which is how such a list writes a name it only wants blocked, are held the same way the `domain` format holds its rules, so a list of that shape costs the same as it would in that format. Only the names carrying an address to answer with are held individually, and those are also what PTR queries are answered from; an unspecified address is shared by every blocked name in a list, so no reverse lookup is recorded for it.
