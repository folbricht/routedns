...
func instantiateGroup(id string, g group, resolvers map[string]rdns.Resolver) error {
	...
	case "cache":
		...
		opt := rdns.CacheOptions{
			...
			PrefetchResolver:    g.PrefetchResolver,
		}
	...
	case "cache-prefetch":
		opt := rdns.CachePrefetchOptions{
			...
			PrefetchSize: 							 g.PrefetchSize, // this should never be bigger than the cache size // defaults to 1000 should be 1/4th of the cache size
		}
